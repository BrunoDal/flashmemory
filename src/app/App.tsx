...
export default function App() {
  ...
  return <>{pwaNotices}<div className="shell"><a className="skip-link" href="#main-content">Aller au contenu</a><header className="topbar"><button className="brand" onClick={() => setView('home')} aria-label="Accueil"><span className="brand-mark">✦</span> flashmemory</button><div className="profile-chip"><span>{profile.name.slice(0, 1).toUpperCase()}</span><select value={profile.id} onChange={e => selectProfile(e.target.value)} aria-label="Profil actif">{profiles.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></div></header>
    <main id="main-content" tabIndex={-1}>{view === 'home' && <Home profile={profile} stats={stats} states={states} activeSession={studySession} onStart={() => void start()} onResume={() => { setIndex(studySession?.currentIndex ?? 0); setView('study'); }} onReview={() => void start('review')} onDiscover={() => void start('discover')} onFree={() => void start('free', CATEGORIES)} onTopic={topic => void start('topic', [topic])} />}
...
function Explore({ states, events, search, setSearch }: { states: ReturnType<typeof useAppData>['states']; events: ReviewEvent[]; search: string; setSearch: (s: string) => void }) {
  ...
  const detailPanelRef = useRef<HTMLDivElement>(null); const detailTitleRef = useRef<HTMLHeadingElement>(null); const selectedTriggerRef = useRef<HTMLButtonElement>(null);
  useEffect(() => {
    if (!selected) return;
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      const focusable = detailPanelRef.current?.querySelectorAll<HTMLElement>('button:not([disabled]), a[href], [tabindex]');
